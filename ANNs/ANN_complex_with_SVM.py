...
def ANN_classifier(Xtrain, ytrain, Xtest, ytest, theta):
    ...
    activation_function = theta.get("activation_function", "relu")  # Get the activation function from theta
    ...
    # Check layer type
    if theta["layer_type"] == "conv1d":
        kernel_size = min(3, X_train.shape[1])  # Ensure kernel size is appropriate
        model.add(tf.keras.layers.Conv1D(filters=60, kernel_size=kernel_size, activation=activation_function, padding='same'))
